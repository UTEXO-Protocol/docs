...
| Dependency | Purpose |
| --- | --- |
| [**bitcoind**](https://github.com/bitcoin/bitcoin/tree/master/doc) (optional) | Chain backend for the `BlockSync` [sync mode](#chain-sync-modes). Not needed with `TransactionSync` |
...
<Note>
  Chain-backend credentials are supplied in the `/unlock` request, not on the command line. See [Unlocking the node](#unlocking-the-node).
</Note>
...
| `--config` | Path to a TOML configuration file (default: `<data-dir>/config.toml` if present). CLI options override file values |
| `--remote-signer-addr` | Address of a [remote signer daemon](/security/rln-remote-signer) (`host:port`). Requires a build with the `remote-signer` feature |

### Unlocking the node

After `POST /init`, every start requires `POST /unlock`. The body selects how LDK syncs with the chain through `ldk_chain_sync`, and sets the RGB wallet's indexer and transport:
...
| --- | --- | --- |
| `BlockSync` | `bitcoind_rpc_username`, `bitcoind_rpc_password`, `bitcoind_rpc_host`, `bitcoind_rpc_port` | You run a trusted bitcoind. More trust-minimized: the node does not rely on an indexer to tell it which transactions are relevant |
| `TransactionSync` | `indexer_url` (Electrum or Esplora) | You don't run bitcoind. The LDK indexer can differ from the RGB wallet's `indexer_url` |

`indexer_url` (the RGB wallet's indexer) is independent of the sync mode, so any indexer can be paired with either mode. `indexer_url` and `proxy_endpoint` can be omitted from the body when they are set in the `[chain]` section of the config file; if no indexer URL is set anywhere, unlock fails with `MissingIndexerUrl`.
...
**Regtest unlock values** (`BlockSync` config, `indexer_url`, `proxy_endpoint`):
...
**Testnet3 unlock values** (`BlockSync` config and `indexer_url`; `proxy_endpoint` only when using proxy transport):
...
RLN does not use xPub request headers or a client-side `*begin`/`*end` PSBT flow. Authenticate to RLN with Biscuit tokens as described below. If your custody model requires keys outside the node, run RLN in external-signer mode; see [Remote Signer](/security/rln-remote-signer).

For complete request and response schemas, see the [RGB Lightning Node API](/rgb-lightning-node/rgb-lightning-node-api).
...
```sh
echo 'role("admin"); check if time($t), $t <= 2027-12-31T00:00:00Z;' \
  | biscuit generate --private-key-file private-key-file -
```
...
| --- | --- | --- |
| `/init` | POST | Initialise the node (creates or imports the mnemonic) |
