...
A running RGB Lightning Node (RLN) exposes a REST JSON API that can be called directly to perform Lightning and RGB operations: channel management, asset issuance, payments, peer connections and more.

The endpoint reference below applies to nodes managed by Utexo Cloud and to self-hosted nodes; the connection and authentication settings differ and are covered separately. Available endpoints and request schemas follow the RLN release a node is running — check the OpenAPI specification for your version.
...
curl -X POST \
  ...
  https://{userId}.thunderstack.org/nodes/{userId}/{nodeId}/issueassetnia
```

### Base URL — self-hosted

The API listens on the port passed to `--daemon-listening-port` (default `3001`). Authenticate with a Biscuit token — see [Authentication](/rgb-lightning-node/self-hosted-rgb-lightning-node#authentication) for issuing and revoking tokens.
...
[https://utexo-protocol.github.io/rgb-lightning-node](https://utexo-protocol.github.io/rgb-lightning-node)