...
Both packages wrap `rgb-lib`, so their on-chain APIs behave the same way — though each owns its own rgb-lib instance and data directory, so state is not shared at runtime.
