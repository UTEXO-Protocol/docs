...
description: 'Complete reference for @utexo/wdk-rgb-lightning — WDK module for RGB Lightning channels, invoices, and payments. Alpha skeleton.'
...
Both packages wrap `rgb-lib`, so their on-chain APIs — UTXO management, asset state, RGB operations — behave the same way, and developers familiar with `wdk-wallet-rgb` will find equivalent calls in `wdk-rgb-lightning`. Each package owns its own rgb-lib instance and its own data directory, though, and they do not share state at runtime: if you need a unified on-chain view across the Lightning node and the on-chain wallet, query each separately and merge in app code.
