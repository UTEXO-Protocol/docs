...
Each node has a unique endpoint URL visible on its dashboard page (`/nodes/{nodeId}`). Connections can be made via mTLS or API token. See \[Connect to RLN Node\](/product-suite/rgb-lightning-node for details.
