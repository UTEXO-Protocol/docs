...
<Info>
  This guide is a **UI walkthrough** for using the Utexo Mint interface. If you want to initiate a mint programmatically using the SDK or REST API, see the [Mint API Reference](/product-suite/mint-api-reference) instead.
</Info>
...
<Info>
  Signing authority is distributed across three independent Federated Signer Nodes, each running inside an AWS Nitro Enclave, using a threshold scheme — no single node can produce a valid signature unilaterally.
</Info>
