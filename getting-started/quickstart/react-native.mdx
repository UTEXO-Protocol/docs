...
Lightning payments require an open channel between your wallet and the node. Use `openChannel()` to request a channel, then verify it is active with `listChannels()` before sending or receiving.

```javascript
import { UTEXOWallet } from '@utexo/rgb-sdk-rn';

// Open a channel with the Utexo LSP node (values in satoshis)
await wallet.openChannel({
  nodeId: '<LSP_NODE_ID>',    // provided by your Utexo node operator
  capacitySats: 100000,
  pushAmountSats: 0,
});

// Verify the channel is active before transacting
...
With an active channel you can send and receive Lightning payments:
...
- [SDK Reference](/product-suite/sdk) — Full method reference for `UTEXOWallet`, including backup and restore
