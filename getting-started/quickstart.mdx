...
<Warning>
  This page documents the archived `@utexo/rgb-sdk` package. New Node.js integrations should use [`@utexo/wdk-rgb-lightning`](/sdk/wdk-rgb-lightning). For browsers see the [Web Quickstart](/getting-started/quickstart/web); for iOS and Android see the [React Native Quickstart](/getting-started/quickstart/react-native).
</Warning>
...
For mainnet endpoint configuration, see the [wdk-rgb-lightning Reference](/sdk/wdk-rgb-lightning).
...
Send `/getbtc` with this address to [@Utexo_RLN_bot](https://t.me/Utexo_RLN_bot) (see [Prerequisites](#prerequisites)). Wait for at least 1 confirmation before continuing.
...
<Info>
  If no assets appear, your wallet may not yet have been assigned any RGB assets. You can also obtain the canonical testnet USDT asset ID from the [wdk-rgb-lightning Reference](/sdk/wdk-rgb-lightning) or by asking in the [Utexo Discord](https://discord.gg/utexo).
</Info>
...
| `createUtxos()` fails | Insufficient BTC balance | Fund your wallet with more testnet BTC via `/getbtc` on [@Utexo_RLN_bot](https://t.me/Utexo_RLN_bot) |
...
- [wdk-rgb-lightning Reference](/sdk/wdk-rgb-lightning) — Current Node.js / Bare module for RGB Lightning
