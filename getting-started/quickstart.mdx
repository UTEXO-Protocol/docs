...
- **A testnet BTC balance** for transaction fees. Get testnet BTC from the Utexo Telegram faucet bot [@Utexo\_RLN\_bot](https://t.me/Utexo_RLN_bot) — send `/getbtc` followed by your Bitcoin address.
...
Send `/getbtc` with this address to [@Utexo\_RLN\_bot](https://t.me/Utexo_RLN_bot) (see [Prerequisites](#prerequisites)). Wait for at least 1 confirmation before continuing. You only need a small amount — a few thousand satoshis is enough to cover fees for this tutorial.
...
Expected result: the receiver's asset list shows a balance of `100` for the USDT asset. If the transfer is still pending confirmation, call `refreshWallet()` again after the next Bitcoin block. RGB transfers require Bitcoin confirmation to finalize. On testnet, a new block typically arrives every 1–10 minutes.
...
| `createUtxos()` fails | Insufficient BTC balance | Fund your wallet with more testnet BTC via `/getbtc` on [@Utexo\_RLN\_bot](https://t.me/Utexo_RLN_bot) |
