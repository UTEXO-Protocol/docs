...
| `blindReceive({...})` | Generate blinded UTXO for receiving |
| `witnessReceive({...})` | Generate witness UTXO for receiving |
| `issueAssetNia({...})` | Issue a new Non-Inflationary Asset |
| `send({...})` | Complete send (begin → sign → end; use `witnessData` for witness invoices; feeRate defaults to 1) |
| `sendBegin({...})` | Prepare transfer (returns unsigned PSBT) |
| `sendEnd({...})` | Complete transfer with signed PSBT |
| `signPsbt(psbt)` | Sign PSBT using wallet mnemonic (async) |
...
| `createBackup({...})` | Create encrypted backup (layer1 \+ utexo in one folder) |
| `vssBackup()` | Backup to VSS (config optional; built from mnemonic \+ default server URL) |
| `vssBackupInfo()` | Get VSS backup info |
| _On-chain_ |  |
| `onchainReceive()` | Generate invoice for depositing from mainnet to UTEXO |
| `onchainSendBegin()` | Start on-chain withdraw (returns unsigned PSBT) |
| `onchainSendEnd()` | Finalize on-chain withdraw with signed PSBT |
...
| `payLightningInvoiceBegin()` | Start Lightning payment (returns unsigned PSBT) |
| `payLightningInvoiceEnd()` | Finalize Lightning payment with signed PSBT |
...
- **Signet:** `rpcs://rgb-proxy.utexo.com/json-rpc`
