...
The interactive reference for these endpoints is the [Resolver API Swagger](https://docs-swap.utexo.com/docs/).
...
<Warning>
  Confirm the request and response payload for this endpoint with Utexo before implementing.
</Warning>

<Note>
  Optimized swaps report only the `Swap` step. Confirm the reporting endpoint for optimized swaps with Utexo during onboarding.
</Note>
