...
The Resolver API is the direction Utexo calls **on the resolver**. Each resolver defines and configures its own callback URLs; there are no prescribed paths.

<Warning>
  Resolver callbacks are not protected by `X-API-Key`. Agree the transport authentication for this direction with Utexo before exposing endpoints.
</Warning>
...
<Note>
  The quote response carries the destination minimum and maximum, expiry in milliseconds, and affiliate and intermediate routing data. Confirm the exact response shape with Utexo during onboarding.
</Note>
...
| `destination_script_hex` | RGB swaps only. Script of your `witness_receive` claim destination, where you will move the RGB allocation when you claim. Not the HTLC script. |

<Note>
  `x_only_public_key`, `refund_control_block`, and `fast_refund_control_block` are only meaningful for `deposit_type=escrowed`.
</Note>

For RGB USDT swaps, the request does not include the asset or amount; load them from the quote. See [RGB USDT in Utexo Swap](/product-suite/swap/rgb#for-resolvers).
...
<Warning>
  Confirm the Solana co-signed callback response shape with Utexo before implementing.
</Warning>
...
<Warning>
  The Bitcoin script field is currently named `tapscipt`. Validate the field name against the current implementation before you wire it up.
</Warning>