...
title: "Mint"
sidebarTitle: "Mint"
description: "Cross-chain stablecoin transfers between EVM, Tron, Solana, Arch, Concordium and the Bitcoin RGB layer."
---
...
Transaction signing is distributed across three independent listener nodes, each running inside its own **AWS Nitro Enclave**. A valid signature requires agreement from at least **2 of the 3 nodes** (threshold scheme). No single node and no external party can unilaterally produce a valid signature.
...
| --- | --- | --- |
| USDT | EVM, Tron, Solana (via USDT0) and <br />Arch, Concordium | USDT on Bitcoin |
