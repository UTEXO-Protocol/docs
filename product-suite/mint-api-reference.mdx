...
<Info>
  **Base URL (testnet / dev):** `https://transfer.stage.utexo.com/api/v0`
...
4. **Mainnet base URL** — The current spec host is empty; `transfer.stage.utexo.com` is the staging environment. The production base URL must be published before this reference goes external.
