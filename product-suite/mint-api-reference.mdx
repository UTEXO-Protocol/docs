...
description: "REST API for bridging assets between EVM networks (Arbitrum) and RGB. Use this reference to discover networks and tokens, estimate fees, initiate transfers, and track transfer status."
---

## Overview

The **Utexo Bridge API** exposes a REST interface for cross-chain asset transfers between EVM-compatible networks (currently Arbitrum) and the RGB protocol on Bitcoin.
...
<Steps>
  ...
  <Step title="Confirm the transaction">
    `POST /transfers/verify-bridge-in` — notify the bridge with the on-chain tx hash to confirm the transfer.
  </Step>
...
<Steps>
  ...
  <Step title="Confirm the transfer">
    `POST /transfers/verify-bridge-in` — notify the bridge that the RGB send has been completed.
  </Step>
  <Step title="Poll transfer status">
...
The `POST /transfers/verify-bridge-in` endpoint validates the caller's identity using a cryptographic signature embedded in the request body:
...
| **`fundsIn()`** | The method on the EVM bridge contract the sender calls to deposit assets for bridging. |
| **transferId** | The bridge's internal identifier for a pre-registered transfer. Used in `verify-bridge-in` and invoice retrieval. |
| **Triggering Tx** | The on-chain transaction that initiates the bridge — the EVM deposit tx (EVM → RGB) or the RGB send (RGB → EVM). |
