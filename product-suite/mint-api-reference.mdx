...
The API is stateless and JSON-based. No authentication header is required. `POST /transfers/verify-bridge-in` and `POST /transfers/submit-transaction` authenticate callers with signature fields embedded in the request body (see [Authentication](#authentication)).
