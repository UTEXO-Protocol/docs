...
<Steps>
  ...
  <Step title="Poll transfer status">
    `GET /transfers/history/{signature-hex}/{pub-key-hex}` — poll history until the transfer reaches a terminal status (`FINISHED` or `FAILED`).
  </Step>
</Steps>
